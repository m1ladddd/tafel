...
def run_test_file(test_file):
    ...
    try:
        ...
        if test_count == 0:
            print(f"⚠️  No tests found in {test_name}")
            return True, 0, 0
        ...
        if result.wasSuccessful():
            print(f"✅ {test_name} PASSED - {passed}/{result.testsRun} tests ({duration:.2f}s)")
            return True, passed, result.testsRun
        else:
            print(f"❌ {test_name} FAILED - {passed}/{result.testsRun} tests passed, {failed} failed ({duration:.2f}s)")
